...
void FindPathStrings (PathStringArray *StringPath, SolutionArray *Maze_solutions, Map *Maze) {
  ...
  StringPath->counter = Maze_solutions->counter;
}
...
void PrintMazePath (Map *TempMaze, SolutionArray *Maze_solutions, Map *Maze, PathStringArray *StringPath, int index) {
  CopyMap(TempMaze, Maze);
  FillAllMaze(TempMaze, 45);
  ConvertPathstoMazePaths(TempMaze, Maze_solutions, index);
...
void displaymessage(Map *Maze, WbDeviceTag dis_sensor[], int step) {
  while (wb_robot_step(TIME_STEP) == -1) {
    continue;
  }
  // Get values from sensors L,R,F
  ...
  printf("Step: %02d, Row: %d, Column: %d, Heading %c, ", step, Maze->curr_row, Maze->curr_col, Maze->pole);
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < StringPath.counter; i++) {
    printf("Path No. %d\n",i+1);
    PrintMazePath(&TempMaze, &Maze_solutions, &Maze, &StringPath, i);
  }
  printf("Best Path:\n");
  PrintMazePath(&TempMaze, &Maze_solutions, &Maze, &StringPath, StringPath.ShortestPathIndex);
  PrintShortestPath(&StringPath);
  printf("Finished Planning\n\n\n\n");

  printf("--- Task 3.2.3 ---\n");
  //displaymessage(&Maze, dis_sensor, 0);
  for (int i = 2; i < StringPath.PathLength[StringPath.ShortestPathIndex]; i++) {
    Action = StringPath.PathStrings[StringPath.ShortestPathIndex][i];
    MoveOneStep(Action, move_tags, &Maze);
    displaymessage(&Maze, dis_sensor, i-2);
    // Need to check walls
  }
